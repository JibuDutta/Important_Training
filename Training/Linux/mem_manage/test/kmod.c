...
long my_ioctl (struct file *fp, unsigned int pid, unsigned long param)
{
	...
	{
		printk("%p--",(void *)ptr->vm_start); /*printing the starting address*/
		printk("%p",(void *)ptr->vm_end); /*Ending address*/
		...
		if(ptr->vm_file != NULL)
		{
			printk(" %p",(void *)ptr->vm_pgoff);
		
			printk(" %lu",ptr->vm_file->f_inode->i_ino);
